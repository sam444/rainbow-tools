...
module.exports = {
     ...
     */
    //绘制dom 元素，生成截图canvas
    doShot: function (traceId, documentId) {
        return new Promise(resolve => {
            let screenShotId = traceId ? traceId : "RAINBOW_SCREENSHOT";
            let id = documentId ? documentId : "registerForm";
            var content = document.querySelector("#"+id); // 需要绘制的部分的 (原生）dom 对象 ，注意容器的宽度不要使用百分比，使用固定宽度，避免缩放问题
            if (content) {
                ...
                html2canvas(content, opts).then(function (canvas) {
                    sessionStorage.setItem(screenShotId, canvas.toDataURL("image/jpeg"));
